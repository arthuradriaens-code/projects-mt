...
xcoordinates = np.linspace(5,500,500)
RelativeAngle = np.zeros(len(xcoordinates))
...
with alive_bar(len(xcoordinates),title='Calculating relative angles',length=20,bar='filling',spinner='dots_waves2') as bar: #fun progress bar, of course not needed for the program to function
    for s,xcoordinate in enumerate(xcoordinates):
        ...
        Average = (direct_angle + angle)/2
        RelativeAngle[s] = degrees(direct_angle - angle)
        BalloonAngle[s] = degrees(direct_angle)

        bar()

plt.plot(BalloonAngle,RelativeAngle)
plt.title("Difference in function of the Direct angle")
plt.xlabel("Direct angle (°)")
plt.ylabel("Direct angle - Reconstructed angle (°)")
plt.show()
