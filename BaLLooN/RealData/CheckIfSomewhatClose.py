...
directory = os.fsencode('/mnt/usb/sonde/gpx/')
with open('SomewhatCloseEvents.csv', 'w', newline='') as csvfile:
    spamwriter = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
    spamwriter.writerow(['Balloon filename (gpx)','station','expected timeframe'])
    for file in os.listdir(directory):
        filename = os.fsdecode(file)
        if (filename[7] == "1") or (filename[7] == "3") or ((filename[9] != "9") and (filename[9] != "8") and (filename[9] != "7") or (filename[-1] != "x")):
        ...
        for StationNumber in stations:
            ...
            for i in range(len(segment['lat'])):
                ...
                HeightDifference = BalloonPosition[2] + 95.5
                Angle = np.arctan(HorizontalDistance/HeightDifference)*180/np.pi
                if (abs(Angle) < 10) and (BalloonPosition[2] < 1000):
    ...
    # Add, markers to the gps-locations of the detectors
                ...
                the_map
    # To save the map:
                the_map.save('MapsSomewhatClose/map_{}_{}.html'.format(filename,StationNumber))
